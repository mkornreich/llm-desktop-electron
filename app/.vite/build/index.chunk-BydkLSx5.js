...
\`\`\`
`}const S=`## Elicitation — collecting skill arguments
...
7. Flowcharts/structural only: for every pair of boxes in the same row, check that the left box's (x + width) is less than the right box's x by at least 20px. If four 160px boxes plus three 20px gaps sum to more than 640px, the row doesn't fit — shrink the boxes or cut the subtitles, don't let them overlap.${o}
...
**The ${e} in viewBox is load-bearing — do not change it.** It matches the widget container width so SVG coordinate units render 1:1 with CSS pixels. With \`width="100%"\`, the browser scales the entire coordinate space to fit the container: \`viewBox="0 0 ${n} H"\` in a ${e}px container scales everything by ${e}/${n} = ${c}×, so your \`class="th"\` 14px text renders at ~${s}px. The font calibration table below and all "text fits in box" math assume 1:1. If your diagram content is naturally narrow, **keep viewBox width at ${e} and center the content** (e.g. content spans x=${r}..${h}) — do not shrink the viewBox to hug the content. This applies equally to inline SVGs inside HTML steppers and widgets: same \`viewBox="0 0 ${e} H"\`, same 1:1 guarantee.
...
`};b(f);const C=e=>{const t=`**Tier packing:** Compute total width BEFORE placing. Example — 4 pub/sub consumer boxes:
- WRONG: x=40,160,260,360 w=160 → 40-60px overlaps (4×160=640 > 480 available)
- RIGHT: x=50,200,350,500 w=130 gap=20 → fits (4×130 + 3×20 = 580 ≤ 590 safe width; right edge at 630 ≤ 640)`,n=`**Structural container example** (library branch with two side-by-side regions, an internal labeled arrow, and an external input). ViewBox 700x320, horizontal layout, color classes handle both light and dark mode — no \`<style>\` block:
\`\`\`svg
<defs>
...
\`\`\``:`For abstract subjects (e.g. transformer attention) at ${e}px: same approach. The five-token fan-out example is omitted — at this width, lay out three tokens (x≈60, 160, 260; w≈80 each) with the middle one as the query, fan three weight lines, and put the caption below at y past the lowest stroke.`;return`## Diagram types
...
:root {
  ...
  --font-mono: var(--font-mono-override,) var(--font-anthropic-mono, "Anthropic Mono Variable"), "Anthropic Mono", "SF Mono", ui-monospace, Menlo, Consolas, monospace;
  --font-mono-ui: var(--font-anthropic-mono, "Anthropic Mono Variable"), "Anthropic Mono", "SF Mono", ui-monospace, Menlo, Consolas, monospace;
      ...
      for (const scripts of phases) {
        for (const oldScript of scripts) {
          ...
          const loaded = willLoad ? new Promise((resolve) => {
            ...
            newScript.onerror = () => {
              sendScriptEvent("load-error", { src: newScript.src });
              resolve();
            };
          }) : Promise.resolve();
...
</html>`}const I=["diagram","mockup","interactive","data_viz","art","chart","elicitation"],L=["mobile","desktop","unknown"];function D(e){const t=b(e),n=C(e),a=k(e);return{diagram:[i,t,n],mockup:[a,i,g],interactive:[a,i,g],data_viz:[a,i,u,m],art:[t,T],chart:[a,i,u,m],elicitation:[S]}}const l="ui://imagine/show-widget.html",M=[{name:"show_widget",description:`Show visual content — SVG graphics, diagrams, charts, or interactive HTML widgets — that renders inline alongside your text response.
