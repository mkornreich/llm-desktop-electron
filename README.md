...
(on-device Ollama), `openrouter`, `cohere`, `gemini`, `mistral`, `groq`, `ollama` (Ollama Cloud) — and any *individual* turn can run on a
...
An old `PROVIDER=openai|local|openrouter|cohere|gemini|mistral|groq|ollama` (in `.provider` or on the command line)
...
| backend the proxy calls | — | the `DEFAULT_PROVIDER` upstream (OpenAI / on-device Ollama / OpenRouter / Cohere / Gemini / Mistral / Groq / Ollama Cloud), plus any provider picked per-turn |
