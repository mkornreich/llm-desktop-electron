...
test("the schema covers every parameter the proxy and launcher read", () => {
  ...
  for (const f of [".provider", ".privacy", ".sync", ".openai-model", ".diagnostics", ".openrouter-model", ".cohere-model"]) {
...
test("every schema entry is well formed", () => {
  for (const s of SCHEMA) {
    ...
    assert.ok(["bool", "int", "text", "enum", "ollama", "ollama-context", "openrouter"].includes(s.type), `bad type on ${s.key}`);
