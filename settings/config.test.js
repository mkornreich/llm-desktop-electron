...
test("the schema covers every parameter the proxy and launcher read", () => {
  ...
  for (const f of [".provider", ".privacy", ".sync", ".openai-model", ".diagnostics", ".openrouter-model", ".cohere-model", ".gemini-model", ".mistral-model", ".groq-model"]) {
...
test("the provider enum is proxy|anthropic with a default-upstream selector", () => {
  ...
  assert.deepEqual(dp.options, ["openai", "local", "openrouter", "cohere", "gemini", "mistral", "groq"]);
});
