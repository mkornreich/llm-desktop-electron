...
const SCHEMA = [
  ...
  { group: "OpenAI model", file: ".openai-model", key: "OPENAI_CLASSIFIER_MODEL", type: "classifiermodel",
  ...
  { group: "OpenAI model", file: ".openai-model", key: "OPENAI_CLASSIFIER_SAFETY_MODEL", type: "composite", classifierChain: true,
    default: "", suggestions: ["freetoken:gemma-4-e2b", "gemini:gemini-3-flash-preview", "cohere:command-a-plus-05-2026", "gpt-5.4-2026-03-05"],
    placeholder: "freetoken:gemma-4-e2b,gemini:gemini-3-flash-preview",
    label: "Auto-mode safety-verdict model(s)",
    help: "An ordered FALLOVER CHAIN for the auto-mode safety verdict — add / remove / reorder members below. The proxy tries them in turn: a member's overflow or error falls over to the next, and EVERY member judges the full transcript (never shortened), so a fallover only tries a different model — it never weakens the check. The verdict fails closed only if ALL members fail. One member = single-shot; EMPTY = use the main model (accept the latency). Each member is a specific provider model you hold a key for, or a local one (freetoken:/local:). Order is the tradeoff: put the on-device gemma FIRST for privacy (it answers most verdicts, ~50s on an 8GB card, and falls over to the cloud only when it overflows/errors — instead of failing closed to manual approval); put a fast big-context cloud model FIRST for ~1-3s verdicts, at the cost of egressing the transcript on every verdict. Claude Code aborts a verdict at 60s and DENIES — a cloud member keeps you well under that. Changing this restarts the proxy." },
  ...
  { group: "Agent behaviour", file: ".openai-model", key: "OPENAI_BARE_MODE", type: "bool",
    default: "0", label: "Bare requests (no system prompt or tools)",
    help: "Sends ONLY the conversation messages upstream on a main turn — strips the whole system prompt and every tool (and tool_choice). For probing a model raw. Classifier and compaction turns are unaffected (they need their prompt). Off by default." },
  ...
  { group: "Compaction", file: ".openai-model", key: "OPENAI_COMPACT_MODEL", type: "text",
    ...
    label: "Summarising model (single, fallback)",
    help: "Used for compaction digests when the chain below is empty. Runs on the default upstream (auto-swapped to that upstream's own model when it isn't OpenAI). Small and fast is the point; it never answers the user. The 'Compaction model chain' below takes precedence when set." },
  { group: "Compaction", file: ".openai-model", key: "OPENAI_COMPACT_MODELS", type: "composite", responsesOnly: true,
    default: "",
    label: "Compaction model chain (ordered, responses-only)",
    help: "A composite/fallback chain for the compaction summariser: the models it tries in turn (each on its OWN provider + key) until one succeeds — so if your first choice is rate-limited or down, compaction still happens. Only models on providers that serve /responses can be chosen (OpenAI / Groq / Ollama Cloud / on-device Local); gemini/cohere/mistral are chat-only and excluded. Empty = the single Summarising model above on the default upstream. Add / remove / reorder with the picker; changing it restarts the proxy." },
...
const PATHS = {
  ...
  OPENAI_BARE_MODE: "agent.bareMode",
