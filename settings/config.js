...
const SCHEMA = [
  ...
  { group: "OpenAI model", file: ".openai-model", key: "OPENAI_CLASSIFIER_SAFETY_MODEL", type: "text",
    default: "", placeholder: "(blank = use the main model)",
    label: "Auto-mode safety-verdict model",
    help: "Blank keeps the MAIN model, which is the safe default. Replaying six real classifier requests from the CLI's error dumps: 3 of 5 verdicts disagreed and every disagreement went the same way — gpt-5.3-codex blocked, gpt-4.1-mini allowed — and on a sixth gpt-4.1-mini emitted no verdict at all. Both answered in about 2 seconds, so downgrading buys nothing. Set a model here only if you accept a weaker security verdict." },
