...
const SCHEMA = [
  ...
  { group: "Agent behaviour", file: ".openai-model", key: "OPENAI_MAX_TRANSPORT_RETRIES", type: "int",
    ...
    help: "A dropped socket is not an answer. undici reports one as TypeError('terminated') mid-stream, and until this existed nothing retried it: the proxy ended the turn, and the CLI could not step in because it had already been handed HTTP 200 and a partial stream — 97 turns in one log died that way. Retried only while NOTHING has been sent to the client yet; past the first delta the turn is kept as-is, because a fresh response would renumber content blocks the client already holds. Never fires for an API refusal or a context-window error, which are answers rather than transport faults. 0 disables it." },
